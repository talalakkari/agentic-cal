...
import { isCalendarInbound, handleCalendarInbound } from "./calendar/inbound";
...
// Shared CORS policy (workers/lib/cors.ts) — same guard is applied to the
// calendar API in workers/calendar/routes.ts so neither surface is unprotected.
...
app.put("/api/v1/mailboxes/:mailboxId", async (c) => {
	...
	const incoming = ((await c.req.json()) as { settings?: unknown }).settings ?? {};
	const parsed = MailboxSettingsSchema.safeParse(incoming);
	if (!parsed.success) {
		return c.json({ error: parsed.error.issues[0]?.message ?? "Invalid settings" }, 400);
	}
	// Merge over existing rather than replacing wholesale, so a partial update
	// can't wipe sibling settings (forwarding / signature / autoReply / prompt).
...
async function receiveEmail(event: { raw: ReadableStream; rawSize: number }, env: Env, ctx: ExecutionContext) {
	...
	// Calendar dispatch (spec §7.1): messages carrying an inbound iTIP method
	// (REPLY/COUNTER/CANCEL) in a text/calendar part divert to the calendar path
	// BEFORE any mailbox logic. "fallthrough" means the payload referenced a UID
	// that isn't one of our blocks — third-party calendar mail that must continue
	// to the inbox. Everything below this branch is the unchanged mailbox hot path.
	if (isCalendarInbound(parsedEmail)) {
		const verdict = await handleCalendarInbound(parsedEmail, rawEmail, env);
