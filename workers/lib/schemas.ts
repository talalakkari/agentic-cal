...
/**
 * Per-mailbox settings persisted to R2 (mailboxes/<id>.json). Validated on
 * create/update so a bad request can't store an arbitrary shape; note that
 * `agentSystemPrompt` is fed verbatim to the agent. Unknown keys pass through
 * for forward-compatibility, but the known fields are type-checked.
 */
export const MailboxSettingsSchema = z
	.object({
		fromName: z.string().optional(),
		forwarding: z
			.object({ enabled: z.boolean(), email: z.string() })
			.partial()
			.optional(),
		signature: z
