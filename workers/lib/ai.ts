...
/** Route Workers AI calls through AI Gateway when configured. */
function aiOptions(env: Env): AiOptions | undefined {
...
export async function verifyDraft(env: Env, body: string): Promise<string> {
	...
	try {
		...
		// Honor the contract: fall back to the ORIGINAL body on AI failure.
		// Returning "" here previously blanked auto-drafted replies on a transient
		// AI Gateway error (the auto-draft path treats an empty result as "nothing
		// to save"). The body is the operator's own content and the verifier is a
		// quality filter, not a safety gate — keeping the original is the safe fail.
		console.error("Draft verifier unavailable, keeping original body:", (e as Error).message);
		return body;
	}
}
