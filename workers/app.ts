...
import { calendarApp } from "./calendar/routes";
import { pollFeeds } from "./calendar/poller";
...
// Calendar API (feeds, poll, events) — CalendarDO only, never MailboxDO
...
export default {
	...
	// Calendar feed poller (cron, every 10 min). Per-feed errors are recorded
	// on the feed row and never throw — a bad feed must not poison the others.
	async scheduled(
